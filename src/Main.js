...
export default class Main extends Component {
	...
	inView = () => {
		document.querySelectorAll('.animateElement').forEach((ele) => {
			...
			if (
				ele_bot_position >= this.state.window_top_position &&
				ele_top_position <= this.state.window_bottom_position
			) {
				setTimeout(() => {
					ele.classList.add(ele.getAttribute('animation'));
				}, 200);
			}
		});
	};
		...
		return (
			<Fragment>
				<main>
					...
					<Header className={this.state.className} click={this.handleClick} offset={this.state.offset} />
